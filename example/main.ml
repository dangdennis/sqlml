...
module Stub = struct
  ...
  let wide =
    [|
      ...
      Sqlml.Value.Null;
      Sqlml.Value.Text "en";
      Sqlml.Value.Text "UTC";
      Sqlml.Value.Text "active";
      Sqlml.Value.Text "owner";
      Sqlml.Value.Text "1234.50";
      Sqlml.Value.Null;
      Sqlml.Value.Int 42;
      Sqlml.Value.Int 0;
      ...
      Sqlml.Value.Null;
      Sqlml.Value.Null;
      Sqlml.Value.Bool false;
      Sqlml.Value.Bool true;
      Sqlml.Value.Text "{}";
      ts;
      ts;
      Sqlml.Value.Null;
    ...
    if contains sql "email_verified_at" then Ok [ wide ]
      ...
      Ok
        [
          ...
          [|
            Sqlml.Value.Text "ann@example.com";
            Sqlml.Value.Text "Hello";
            Sqlml.Value.Int 3;
          |];
        ]
    else
...
let () =
  ...
  (match
     search_users conn ~organization_id:id ~email_pattern:"%@example.com" ~limit:10
   with
  | Ok rows ->
      Printf.printf "search : %d row(s), first = %s\n" (List.length rows)
        (List.hd rows).email
  ...
  | Ok rows ->
      Printf.printf "summary: %d row(s), first = %s\n" (List.length rows)
        (List.hd rows).Wide.email
