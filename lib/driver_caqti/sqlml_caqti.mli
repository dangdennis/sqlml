...
(** Connection URI from [DATABASE_URL], or from
    [PGHOST]/[PGPORT]/[PGUSER]/[PGDATABASE]. *)
val uri_of_env : unit -> Uri.t

(** A single pooled-less connection, scoped to the switch. Prefer {!Pool}. *)
val connect :
  sw:Eio.Switch.t -> stdenv:Caqti_eio.stdenv -> Uri.t -> (Sqlml.conn, Sqlml.Error.t) result

module Pool : sig
  (** A pool carries no query operations on purpose. The only way to reach a
      {!Sqlml.conn} is {!use} or {!transaction}, both of which scope it, so a
      connection cannot outlive its borrow and a transaction cannot leak
      statements onto a different connection. *)
  type t
  ...
  (** Borrow a connection and run [f] on it inside a transaction. Pinning to one
      connection is why this belongs on the pool rather than being assembled by
      the caller. See {!Sqlml.transaction} for commit and rollback behaviour. *)
  val transaction : t -> (Sqlml.conn -> ('a, Sqlml.Error.t) result) -> ('a, Sqlml.Error.t) result
end
